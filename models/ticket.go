...
func CreateTicket(subject string, userID int64) (int64, error) {
	var id int64
	err := db.Postgres.QueryRow(`
...
func GetRandomOpenTicket() (*Ticket, error) {
	var t Ticket
	err := db.Postgres.QueryRow(`
		SELECT id, user_id, admin_id, status, subject, created_at, updated_at
		FROM tickets
...
func AssignTicketAdmin(ticketID, adminID int64) error {
...
func AddUserToTicket(ticketID, userID int64) error {
    query := `
        UPDATE tickets
        ...
        WHERE id = $2
    `
    _, err := db.Postgres.Exec(query, userID, ticketID)
    if err != nil {
...
func GetMessagesForTicket(ticketID, userID int64) ([]TicketMessage, error) {
    var ticket struct {
    ...
    err = db.Postgres.Select(&messages, `
        SELECT *
...
func GetTicketsForUser(userID int64) ([]Ticket, error) {
    var tickets []Ticket
    query := `
        SELECT *
        FROM tickets
    ...
    `
    err := db.Postgres.Select(&tickets, query, userID)
    if err != nil {
...
func ExitFromTicket(ticketID, userID int64) error {
    var ticket struct {
        UserID          *int64  `db:"user_id"`
        AdminID         *int64  `db:"admin_id"`
        AdditionalUsers pq.Int64Array `db:"additional_users_have_access"`
...
func OpenTicket(ticketID int64) error {
	_, err := db.Postgres.Exec(`
		UPDATE tickets
...
func PendingTicket(ticketID int64) error {
	_, err := db.Postgres.Exec(`
...
func AddTicketMessage(ticketID, senderID int64, text string) error {
	_, err := db.Postgres.Exec(`
...
func GetTicketMessages(ticketID int64) ([]TicketMessage, error) {
...
func MarkTicketMessagesRead(ticketID int64, userID int64) error {
	_, err := db.Postgres.Exec(`
