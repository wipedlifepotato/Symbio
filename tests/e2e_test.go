...
func httpPostJSON(t *testing.T, path string, payload any, headers map[string]string) (*http.Response, []byte) {
    ...
    if payload != nil {
        if err := json.NewEncoder(&buf).Encode(payload); err != nil { t.Fatalf("encode: %v", err) }
    }
...
func rdbFromEnv(t *testing.T) *redis.Client {
    ...
    if addr == "" { t.Skip("REDIS_ADDR not set; skipping captcha-dependent tests") }
...
func Test_Register_Auth_Wallet_E2E(t *testing.T) {
    ...
    if captchaID == "" { t.Fatalf("no X-Captcha-ID header") }
    ...
    resp, _ = httpGet(t, "/captcha", nil)
    if resp.StatusCode != http.StatusOK { t.Fatalf("captcha 2 status=%d", resp.StatusCode) }
    captchaID2 := resp.Header.Get("X-Captcha-ID")
    if captchaID2 == "" { t.Fatalf("no X-Captcha-ID for auth") }
    ...
    if authResp.Token == "" { t.Fatalf("empty token") }
